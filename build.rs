...
fn main() {
    ...
    let (bpf_arch, multiarch_include, arch_include) = match target_arch.as_str() {
        "x86_64" => ("x86", "x86_64-linux-gnu", None),
        "aarch64" => ("arm64", "aarch64-linux-gnu", Some("bpf/arm64")),
    ...
    if let Some(include) = arch_include {
        clang_args.push(format!("-I{include}"));
    }
    clang_args.push("-Ibpf".into());
    ...
    println!("cargo:rerun-if-changed=bpf/arm64/vmlinux.h");
